...
- ***A criteria is pushed down to CouchDB's Mango, and what it cannot read is decided by `jsongin`.*** This is the same translator `jsonstor-mongodb` uses, narrowed. An equality - `$eq` and the plain `{ field: value }` form - is decided by the server, along with `$and`, `$or`, `$exists` and `$size`; the four comparisons, `$in`, `$mod`, `$ne`, `$nin` and `$elemMatch` narrow the search and `jsongin` settles the answer; `$not`, `$nor`, `$regex`, `$all` and `$type` are decided by `jsongin`. The answer is the same one either way - what a dropped operator costs is a document read, never a wrong result.
- ***Two of CouchDB's answers differ from MongoDB's, and the adapter asks each one a second way rather than losing a document.***
  ...
  - ***CouchDB answers a condition only where the whole path resolves***, where `jsongin` and MongoDB also return the documents which lack the field, or hold a scalar or an array where the path expects an object. A negation, an `$exists: false`, and an equality, a comparison or an `$in` whose operand mentions `null` are each rendered prefix by prefix - the leaf, and at every prefix a non-object none of whose elements yields the value - which was measured to agree with `jsongin` exactly on both versions.
  - ***CouchDB's `$mod` answers for integers only***, where `jsongin` and MongoDB truncate the value first. The adapter offers every fractional number beside the condition and `jsongin` decides those. And its `$in` compares an array field's elements and never the array itself, so an array member is offered as a whole-array equality beside it.
